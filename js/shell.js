...
class Shell {
  ...
  async _cmdAgents(args) {
    ...
    for (const a of this._agents) {
      ...
      this.term.write('      Підозра: [', 'default');
      this.term.write(sBar, sColor);
      this.term.writeln(`] ${a.suspicion}%`, 'default');
  ...
  async _cmdCreate(args) {
    ...
    for (let i = 0; i < typesMenu.length; i++) {
      ...
      this.term.write(`  ${letter}) ${label.padStart(10)}  `, 'default');
      this.term.write(String(cpu), cpuOk ? 'default' : 'red');
      this.term.write('%  ', 'default');
      this.term.write(String(ram), ramOk ? 'default' : 'red');
      this.term.writeln(`%  ${desc}`, 'default');
    }
    this.term.print('');
